(function($){
     ...
     *
     * Changelog :
     * 2012/10/1 - Brice Favre <brice.favre@blogspirit.com> Improve usability and fix a bug on tinyMCE
     */
    $.fn.editable = function(options){
        var defaults = {
            onEdit: null, // function|null : function to call on edition
            onSubmit: null, // function|null : function to call when we submit form
            onCancel: null, // function|null : function to call when we cancel form
            editClass: null, // string : Classname to add on
            submit: null, // Libelle of the submit
            cancel: null, // Libelle of the cancel
        ...
        options.toEditable = function(){
            ...
            // Cancel Event
            if(opts.cancel) {
                $('<button class="btn btn-small"/>').appendTo($this)
                    .html(opts.cancel)
                    .one('mouseup',function(){opts.toNonEditable($(this).parent(),false)});
            }
            // Submit Event
            if(opts.submit){
                $('<button class="btn btn-small btn-primary"/>').appendTo($this)
                    .html(opts.submit)
        ...
        'wysiwyg': {
            toEditable: function($this,options){
                if(typeof(FCKeditor) != "undefined" && options.editor instanceof FCKeditor){
                    ...
                    //tinyMCE
                    $('<textarea name="'+options.editor.id+'" id="'+options.editor.id+'_editor" style="width:100%" />').appendTo($this)
                        .val($this.data('editable.current'));

                    // ugly but unavoidable because the way tiny works.
                    var ed = new tinymce.Editor(options.editor.id+'_editor', {});
